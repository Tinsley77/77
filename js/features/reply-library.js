...
// 造词库
if (typeof window.wordBank === 'undefined') window.wordBank = [];
if (typeof window._wbCleanStreak === 'undefined') window._wbCleanStreak = 0;
...
async function saveWordBank() {
    try {
        await localforage.setItem(getStorageKey('wordBank'), window.wordBank);
        await localforage.setItem(getStorageKey('wbMeta'), {
            cleanStreak: window._wbCleanStreak,
            cleanThreshold: window._wbCleanThreshold
        });
    } catch(e) {}
}
async function loadWordBank() {
    try {
        ...
        if (meta) {
            ...
            window._wbCleanThreshold = meta.cleanThreshold || 200;
        }
    } catch(e) {}
}

// 造词库动态阈值清理（每次触发回复时调用）
async function _maybeCleanWordBank() {
    if (window.wordBank.length < window._wbCleanThreshold) return;
    const roll = Math.random();
    if (roll < 0.03) {
        // 3%：删1-2条，计数归零，阈值维持当前值
        const deleteCount = Math.random() < 0.5 ? 1 : 2;
        for (let i = 0; i < deleteCount && window.wordBank.length > 0; i++) {
            window.wordBank.splice(Math.floor(Math.random() * window.wordBank.length), 1);
        }
        window._wbCleanStreak = 0;
        await saveWordBank();
    } else if (roll < 0.23) {
        // 20%：触发检查但不删，连续计数+1
        window._wbCleanStreak++;
        if (window._wbCleanStreak >= 6) {
            window._wbCleanThreshold += 100;
            window._wbCleanStreak = 0;
        }
        await saveWordBank();
    }
...
// 造词库自动生成定时器（每1/2/3小时随机触发一次，主字卡≥200条才生成）
function _scheduleWordBankAuto() {
    if (window._wbAutoTimer) clearTimeout(window._wbAutoTimer);
    const roll = Math.random();
    const hours = roll < 0.2 ? 1 : (roll < 0.6 ? 2 : 3);
    window._wbAutoTimer = setTimeout(async () => {
        if (customReplies.length >= 200) {
            const cnt = Math.floor(Math.random() * 8) + 1; // 1-8条
            const parts = [];
            for (let k = 0; k < cnt; k++) {
                parts.push(String(customReplies[Math.floor(Math.random() * customReplies.length)]).trim());
            }
...
function renderWordBankTab(container) {
    ...
    selBar.querySelector('#wb-sel-del').onclick = async () => {
        if (!selected.size) return;
        if (!confirm(`确定删除所选 ${selected.size} 条吗？`)) return;
    ...
    container.querySelectorAll('.wb-del-btn').forEach(btn => {
        btn.onclick = async (e) => {
            ...
            window.wordBank.splice(parseInt(btn.dataset.idx), 1);
            await saveWordBank();
            renderReplyLibrary();
        };
    });
...
function _showExportUI() {
    ...
    const modules = [
        ...
        { id: '_re_wordbank', icon: ICONS.comment,   label: '造词库',        count: (window.wordBank||[]).length,             key: 'wordBank' },
...
function _doExport(selectedModules) {
    ...
    selectedModules.forEach(m => {
        ...
        else if (m.key === 'wordBank')            { libraryData.wordBank           = window.wordBank || [];               libraryData.modules.push('wordBank'); }
...
function _normalizeImportData(data) {
    ...
    const knownKeys = ['customReplies','customPokes','customStatuses','customMottos','customIntros','customEmojis',
                       ...
                       'announcementConfig','announcementText','announcementStatusPool','wordBank'];
...
function _showImportUI(data) {
    ...
    const allModules = [
        ...
        { id: '_ri_wordbank', icon: ICONS.comment,   label: '造词库',        data: data.wordBank,            key: 'wordBank' },
        ...
        try {
            ...
            if (overwrite) {
                selected.forEach(m => {
                    ...
                    else if (m.key === 'wordBank') { window.wordBank = data.wordBank; saveWordBank(); totalAdded += data.wordBank.length; }
                ...
                selected.forEach(m => {
                    ...
                    } else if (m.key === 'wordBank') {
                        const before = (window.wordBank||[]).length;
                        window.wordBank = [...(window.wordBank||[]), ...data.wordBank];
                        saveWordBank();
                        totalAdded += window.wordBank.length - before;
...
function _showWordBankBatchAddDialog() {
    const overlay = _makeOverlay();
    const panel = document.createElement('div');
    panel.style.cssText = `
        background:var(--secondary-bg);border-radius:22px;padding:24px;
    ...
    `;
    panel.innerHTML = `
        <div style="font-weight:500;font-size:15px;margin-bottom:14px;color:var(--text-primary);">批量添加造词库</div>
        <textarea id="wb-batch-input" rows="8" placeholder="每行一条内容，批量添加到造词库…"
            style="width:100%;box-sizing:border-box;padding:12px;border-radius:12px;border:1px solid var(--border-color);
            background:var(--primary-bg);color:var(--text-primary);font-size:13px;
            font-family:var(--font-family);outline:none;resize:none;line-height:1.6;flex:1;"></textarea>
        <div style="display:flex;gap:10px;margin-top:14px;">
            <button id="wb-batch-cancel" class="modal-btn modal-btn-secondary" style="flex:1;">取消</button>
            <button id="wb-batch-confirm" class="modal-btn modal-btn-primary" style="flex:2;">添加</button>
        </div>
    `;
    overlay.appendChild(panel);
    document.body.appendChild(overlay);
    panel.querySelector('#wb-batch-cancel').onclick = () => overlay.remove();
    panel.querySelector('#wb-batch-confirm').onclick = async () => {
...
function initReplyLibraryListeners() {
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            ...
            if (currentSubTab === 'wordbank') {
                _showWordBankBatchAddDialog(); return;
            }
